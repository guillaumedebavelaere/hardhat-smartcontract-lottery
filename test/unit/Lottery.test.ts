...
!developmentChains.includes(network.name)
    ...
    : describe("Lottery unit tests", () => {
        let lottery: Lottery, vrfCoordinatorV2Mock: VRFCoordinatorV2Mock, lotteryEntranceFee: BigNumber, interval: BigNumber;
        ...
        describe("enterLottery", () => {
            it("reverts when you don't pay enough", async () => {
                await expect(lottery.enterLottery({ value: ethers.utils.parseEther("0") })).to.be.revertedWithCustomError(
                    lottery,
            ...
            it("emits a LotteryEntered event", async () => {
                await expect(lottery.connect(playerSigner).enterLottery({ value: lotteryEntranceFee }))
                    .to.emit(lottery, "LotteryEntered").withArgs(player)
            });
          ...
          // All the assertions are done once the WinnerPicked event is fired
            it("picks a winner, resets, and sends money", async () => {
                ...
                await new Promise<void>(async (resolve, reject) => {
                    lotteryConnect.once("WinnerPicked", async () => { // event listener for WinnerPicked
